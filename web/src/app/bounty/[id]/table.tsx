...
export function ChildBountiesTable({rows, chain}: {rows: ChildBountyRow[]; chain: ChainProps}) {
    const columns = useMemo(() => {
        ...
        return col.columns([
            ...
            col.display({id: 'status', header: 'Status', meta: {align: 'right'}, cell: ({row}) => <Badge variant={bountyStatusTone(row.original.status)}>{bountyStatusLabel(row.original.status)}</Badge>}),
        ])
    }, [chain])
